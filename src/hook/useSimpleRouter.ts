...
export default function useSimpleRouter() {
   ...
   */
  const pushState = useCallback((key: string, value: string) => {
   ...
   * @param {string} value - Parameter value
   */
  const replaceState = useCallback((key: string, value: string) => {
    const url = new URL(window.location.href)
    url.searchParams.set(key, value)
   ...
   */
  function Route({ depth = 0, path, element, children }: RouteProps) {
    ...
    const injectDepth = (child: ReactNode) => {
      if (React.isValidElement(child) && child.type === Route) {
        return React.cloneElement(child as React.ReactElement<any>, { depth: depth + 1 })
      }
      return child
    }
